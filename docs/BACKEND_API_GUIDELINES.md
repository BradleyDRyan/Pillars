...
- `POST /api/ai/claude/tools/stream` - Stream Claude responses with server-side tools (plans, clarifications, web search/fetch). Emits SSE events: `content`, `tool_call`, `tool_result`, `done`.
...
Set the following environment variables before using the new tool framework:

- `ANTHROPIC_API_KEY` – required for Claude access (existing)
- `CLAUDE_MODEL` – optional override for the Claude model used in tool runs
- `BRAVE_SEARCH_API_KEY` – enables the `web_search` tool via Brave Search

Add them to `.env.local` for development and to your production/Vercel environment configuration. Missing keys will disable or degrade the related tools.
