...
router.post('/token', verifyToken, async (req, res) => {
  try {
    ...
    const response = await axios.post('https://api.openai.com/v1/realtime/sessions', {
        model: 'gpt-realtime',
        voice: voice,
        instructions: `You are a helpful assistant. ${speakingSpeed === 'very_fast' ? 'Speak very quickly and efficiently.' : speakingSpeed === 'fast' ? 'Speak quickly and concisely.' : speakingSpeed === 'slow' ? 'Speak slowly and clearly.' : 'Speak at a normal pace.'}
...
Important: when you are describing something the user just showed you in an image, do NOT call extract_entries. The photo pipeline already creates the entry. Simply describe the image conversationally.

You will receive image uploads accompanied by short prompts such as “This is the photo I just took.” Treat that as the user handing you the image for context. Do not rephrase the image into a new entry unless the user explicitly asks to save something beyond the photo description.
