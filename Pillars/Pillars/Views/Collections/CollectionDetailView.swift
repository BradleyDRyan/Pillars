...
struct CollectionDetailView: View {
    ...
    init(collection: Collection, hideTabBar: Binding<Bool>) {
                ...
                if viewModel.isLoading {
                    ...
                    VStack(spacing: 12) {
                        ForEach(viewModel.entries) { entry in
                            let collectionEntry = viewModel.collectionEntries.first(where: { $0.entryId == entry.id })

                            NavigationLink {
                                CollectionEntryDetailView(entry: entry, collectionEntry: collectionEntry, collection: collection)
                            } label: {
